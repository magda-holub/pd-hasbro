...
//select product group for carousel
...
function removeAddons() {
	var ppAddons = document.querySelectorAll('#celek .ppAddons');

	ppAddons.forEach(function (addon) {
		addon.remove();
	});

	console.log('remove addons');
};
...
document.addEventListener("DOMContentLoaded", function() {
	if (window.location.href.includes("admin") || window.location.href.includes("127")) {
	  ...
	  brandSelect();
	  videoSelect();
      runCarousel(); /*for preview outside of admin*/
	} else {
	  runCarousel();
	  removeAddons();
	}
